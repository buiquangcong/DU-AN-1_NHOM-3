...
class HdvQuanLyTourController
{
    ...
    {
        $this->modelTour = new HdvQuanLyTour(); // Khởi tạo model Tour
        $this->modelDanhMuc = new HdvDanhMuc();
        $this->modelNhaCungCap = new HdvNhaCungCap();

        // $this->modelNhaCungCap = new HdvNhaCungCap(); // Khởi tạo model NCC
    }
    ...
    public function postAddTour() // Đổi tên hàm
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($error)) {
                ...
                $upload_dir_web = BASE_URL_HDV . '/assets/uploads/';
                ...
                if (move_uploaded_file($fileAnhBia['tmp_name'], $target_file_fs)) {
                    ...
                    $_SESSION['error'] = $error;
                    header('Location: ' . BASE_URL_HDV . '?act=add-tour');
                    exit;
                }
                ...
                if ($insertId) {
                    ...
                    header('Location: ' . BASE_URL_HDV . '?act=list-tours');
                    exit;
                } else {
